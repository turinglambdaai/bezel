...
- the machine-readable Qt redistribution policy, exact QtBase 6.8.4 source hash, and reviewed official patch hashes;
...
- publication of the exact verified QtBase base source and applied patch bundle beside public binaries;
- release SHA-256 generation.

Every public runtime is built from that same verified source-and-patch set as shared libraries with ICU disabled and QtBase's bundled third-party copies selected where supported. Linux keeps non-Qt host libraries external; macOS disables the obsolete OpenGL/AGL path; Windows excludes compiler, DXC, D3D compiler, and software-OpenGL redistributables. This makes each binary/source relationship explicit and prevents public online binaries from silently drifting away from the audited security level.
...
Bezel itself is MIT licensed. **Bezel's public prebuilt GitHub runtime path is intentionally Qt LGPLv3-only and dynamically linked.** It ships required notices/relinking material and publishes the exact verified QtBase base source and applied official patches with the release. The public workflow refuses a release that is merely labelled "commercial Qt" while being built from public Qt artifacts.

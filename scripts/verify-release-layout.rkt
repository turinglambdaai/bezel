...
(case (system-type)
  [(windows)
   (for ([name (in-list '("bezel.dll" "Qt6Core.dll" "Qt6Gui.dll" "Qt6Widgets.dll"))])
     (unless (file-exists? (build-path root name))
       (error 'verify-release-layout "missing required Windows runtime file: ~a" name)))
   (unless (file-exists? (build-path root "platforms" "qwindows.dll"))
     (error 'verify-release-layout "missing Windows QPA plugin platforms/qwindows.dll"))]
  [(macosx)
  ...
  [else
   ...
   (unless (directory-exists? (build-path root "platforms"))
     (error 'verify-release-layout "missing Linux platform plugin directory"))])
